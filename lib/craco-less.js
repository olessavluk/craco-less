...
const overrideWebpackConfig = ({ context, webpackConfig, pluginOptions }) => {
  ...
  let lessRule = pluginOptions.modules
    ? {
  ...
  loaders.forEach(ruleOrLoader => {
    ...
    if (
      ...
      throwError(
        `Found an unhandled loader in the ${context.env} webpack config: ${
          rule.loader
        }`,
  ...
  if (pluginOptions.modules) {
    fileLoaderMatch.loader.exclude.push(lessModuleRegex);
  } else {
    fileLoaderMatch.loader.exclude.push(lessRegex);
  }

  // semantic-ui specific extensions
